...
def process_rumination(raw_rumination_data):
    ...
    rum_df['observation_time'] = rum_df['observation_time'].apply(time_str_to_datetime)
    ...
    rum_df_sum.groupby(['Date'], as_index=False).agg({'rumination_count': 'sum', 'eating_count': 'sum', 'total_rumination': 'sum', 'total_eating': 'sum'})
    return rum_df_sum
